...
public class DataConverter {
	...
	public static Object mapToObject(Map<String, Object> map, Object obj) {
        ...
        while(itr.hasNext()) {
            ...
            // debug
//            System.out.println("keyAttribute: " + keyAttribute + ", methodString: " + methodString + ", map.get(keyAttribute).getClass(): " + map.get(keyAttribute).getClass());
                ...
                if(methodString.equalsIgnoreCase(methods[i].getName())) {
                    try {
                    	...
                    	// java.sql.Timestamp to LocalDate
                    		...
                    		methods[i].invoke(obj,
                    				((java.sql.Timestamp) map.get(keyAttribute)).toLocalDateTime().toLocalDate());
                    	}
                    	// oracle.sql.TIMESTAMP to LocalDateTime
                    	else if(map.get(keyAttribute) instanceof oracle.sql.TIMESTAMP) {
                    		methods[i].invoke(obj,
                    				((oracle.sql.TIMESTAMP) map.get(keyAttribute)).timestampValue().toLocalDateTime());
                    	}
                    	
//                    	// java.sql.Date to LocalDate
//                    	else if(map.get(keyAttribute) instanceof java.sql.Date) {
//                    		methods[i].invoke(obj, ((Date) map.get(keyAttribute)).toLocalDate());
//                    	}
//                    	// java.sql.Timestamp to LocalDateTime
//                    	else if(map.get(keyAttribute) instanceof java.sql.Timestamp) {
//                    		methods[i].invoke(obj, ((Timestamp) map.get(keyAttribute)).toLocalDateTime());
//                    	}
                    	else
                    		methods[i].invoke(obj, map.get(keyAttribute));
                    } catch(Exception e) {
